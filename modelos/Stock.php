  ...
  public function getTablesBases($marca){
    ...
    foreach ($resultado as $value) {
        ...
        foreach ($grads as $key) {
         ...
         $html .= "<tr class='filasb'><td colspan='50' style='text-align: center;cursor: pointer;' onClick='action_gradsbasevs(\"".$titulo."\",\"".$key."\",\"".$codigo."\",".$id_tabla.",\"".$marca."\")'>".$key."</td><td colspan='50' id=".$id_td." style='text-align: center;cursor: pointer;' onClick='initStockBasesvs(\"".$key."\",\"".$codigo."\",".$id_tabla.",\"".$marca."\",\"".$diseno."\",\"".$id_td."\");'>".$stock."</td></tr>";
...
public function eliminarCodigoBaseVs($codigo,$base,$id_tabla){ 
    ...
    $sql = "delete from grad_tablas_base where graduacion=? and id_tabla_base=?;";  
    $sql = $conectar->prepare($sql);
    $sql->bindValue(1, $base);
    $sql->bindValue(2, $id_tabla);
    $sql->execute();
    ...
    $sql3 = "delete from stock_bases where codigo=? and base=? and id_tabla_base=?;";
    $sql3 = $conectar->prepare($sql3);
    $sql3->bindValue(1, $codigo);
